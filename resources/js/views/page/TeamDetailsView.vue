...
import Item from './parts/TeamSingle.vue';
...
async function load() {
	let res = await axios.get('/web/api/blog/teams/' + route.params.slug ?? 'error');
	item.value = res?.data;
	console.log(item);
}
</script>
...
<template>
	<Layout title="Team" :description="item.name">
		<Item :obj="item" />
		<ReferencesSlider />
	</Layout>

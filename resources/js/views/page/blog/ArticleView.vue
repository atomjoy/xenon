...
import ArticleLayout from '@/layouts/page/BlogLayout.vue';
...
<template>
	<ArticleLayout :title="$t('Article')" :description="article.title">
		<div class="blog_article">
			...
			<div class="blog_article_date">
				...
				<span class="blog_article_category" v-for="c in article.categories">
					<a :href="'/category/' + c.slug" target="_blank">#{{ c.name.toLowerCase() }}</a>
				</span>
			...
			<div class="blog_article_tags">
				<span class="blog_article_tag" v-for="c in article.tags">
					<a :href="'/tag/' + c.slug" target="_blank">{{ c.slug.toLowerCase() }}</a>
				</span>
			</div>
			...
			<div class="author_articles" v-if="article?.author?.articles.length > 0">
				...
				<div class="author_article_small" v-for="a in article?.author?.articles">
					<a :href="'/article/' + a.slug" :title="a.title">
			...
			<Comments :article="article" :paginate="article?.comments?.paginate" />
		</div>
	</ArticleLayout>

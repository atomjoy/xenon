...
export const useItemStore = defineStore('service', () => {
	// State
	...
	async function loadList() {
		let res = await axios.get('/web/api/admin/services?page=' + current_page.value + '&perpage=' + perpage.value);
	...
	async function deleteItem(id) {
		try {
			if (confirm('Delete ?')) {
				let res = await axios.delete('/web/api/admin/services/' + id);
				setMessage(res);
	...
	async function createItem(e) {
		try {
			let res = await axios.post('/web/api/admin/services', new FormData(e.target));
			setMessage(res);
			resetForm(e);
		} catch (err) {
			setError(err);
	...
	async function updateItem(id, data) {
		try {
			...
			let res = await axios.post('/web/api/admin/services/' + id, data);
			setMessage(res);
		} catch (err) {
			setError(err);
		}
	...
	async function loadItem(id) {
		try {
			let res = await axios.get('/web/api/admin/services/' + id);
			item.value = res?.data;
		} catch (err) {
			setError(err);
		}
	...
	async function removeImage(id) {
		try {
			let res = await axios.get('/web/api/admin/services/remove/' + id);
			setMessage(res);
			await loadItem(id);

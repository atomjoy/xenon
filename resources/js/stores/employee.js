...
export const useItemStore = defineStore('employee', () => {
	// State
	...
	async function loadList() {
		let res = await axios.get('/web/api/admin/employees?page=' + current_page.value + '&perpage=' + perpage.value);
	...
	async function deleteItem(id) {
		try {
			if (confirm('Delete ?')) {
				let res = await axios.delete('/web/api/admin/employees/' + id);
				setMessage(res);
	...
	async function createItem(e) {
		try {
			let res = await axios.post('/web/api/admin/employees', new FormData(e.target));
			setMessage(res);
			resetForm(e);
		} catch (err) {
			setError(err);
	...
	async function updateItem(id, data) {
		try {
			...
			let res = await axios.post('/web/api/admin/employees/' + id, data);
			setMessage(res);
		} catch (err) {
			setError(err);
		}
	...
	async function loadItem(id) {
		try {
			let res = await axios.get('/web/api/admin/employees/' + id);
			item.value = res?.data;
		} catch (err) {
			setError(err);
		}
	...
	async function removeImage(id) {
		try {
			let res = await axios.get('/web/api/admin/employees/remove/' + id);
			setMessage(res);
			await loadItem(id);
